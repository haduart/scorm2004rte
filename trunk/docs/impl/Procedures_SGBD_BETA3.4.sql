...
DELIMITER $
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertItems`( IN _item_id_text VARCHAR(20), IN _leaf BIT(1), 
		IN _title VARCHAR(200), IN _father_item_id integer, IN _organization_id integer, 
		IN _is_visible BIT(1))
BEGIN
INSERT INTO items SET item_id_text = _item_id_text, 
	leaf = _leaf, title = _title, 
	father_item_id = _father_item_id, 
...
#CALL insertOptional_data_items( _item_id integer, _parameters VARCHAR(500), _dataFromLMS VARCHAR(500), _completionThreshold DECIMAL(5,3), _timeLimitAction_exit BIT(1), _timeLimitAction_message integer, _max_time_allowed integer);
DROP PROCEDURE IF EXISTS `insertOptional_data_items`;
DELIMITER $
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertOptional_data_items`( IN _item_id integer, IN _parameters VARCHAR(500), IN _dataFromLMS VARCHAR(500), IN _completionThreshold DECIMAL(5,3), IN _timeLimitAction_exit BIT(1), IN _timeLimitAction_message integer, IN _max_time_allowed integer)
BEGIN
	INSERT INTO optional_data_items SET parameters = _parameters, dataFromLMS = _dataFromLMS, completionThreshold = _completionThreshold, timeLimitAction_exit = _timeLimitAction_exit, timeLimitAction_message = _timeLimitAction_message, max_time_allowed = _max_time_allowed;
	UPDATE items 
...
#CALL insertObjectives( _adlseqSequencing_id integer, _objective_name VARCHAR(20), _item_id integer, _objective_father integer, _satisfiedByMeasure BIT(1), _minNormalizedMeasure DECIMAL(3,2));
DROP PROCEDURE IF EXISTS `insertObjectives`;
DELIMITER $
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertObjectives`( IN _adlseqSequencing_id integer, IN _objective_name VARCHAR(20), IN _item_id integer, IN _objective_father integer, IN _satisfiedByMeasure BIT(1), IN _minNormalizedMeasure DECIMAL(3,2))
BEGIN
	INSERT INTO objectives SET objective_name = _objective_name, item_id = _item_id, objective_father = _objective_father, satisfiedByMeasure = _satisfiedByMeasure, minNormalizedMeasure = _minNormalizedMeasure;
	SELECT LAST_INSERT_ID(); 
...
#CALL insertStudentsClass( _organization_id integer, _global_student_id integer);
