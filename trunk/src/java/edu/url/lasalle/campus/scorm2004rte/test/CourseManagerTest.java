...
/**
* $Id: CourseManagerTest.java,v 1.18 2008/01/10 06:58:29 ecespedes Exp $
* <b>Títol:</b> CourseManagerTest<br><br>
* <b>Descripció:</b> JUnit Test Class.<br><br> 
*
* @author NomAutor / Enginyeria La Salle / eMail
* @version Versió $Revision: 1.18 $ $Date: 2008/01/10 06:58:29 $
...
public class CourseManagerTest extends TestCase {
	...
	public final void testCourseManager() {
		...
		/**
		 * TODO: TEST 2
		 * Farem unes crides per saber l'estat del seqüenciament
		 ...
		 * TODO: TEST 3
		 * Farem unes crides per saber l'estat del seqüenciament
		 * (attempted, completed, satisfied, measure, etc) però
		 * aquest cop des d'un item on els estats sigui diferents.
		 * 
		 * El cluster estarà format per ASSETS.
		 */
		System.out.println("\n################## TEST 3 ##################" 
		 ...
		 * aquest cop des d'un item on els estats sigui diferents.
		 * 
		 * El cluster estarà format per SCO's.
		 */
		System.out.println("\n################## TEST 4 ##################" 
		 ...
		 * rebre SCO. Això comporta els pasos de:
		 * 
		 ...
		 * PAS 4: Comprovar si té PostCondition o exitCondition.
		 * PAS 5: Comprovar si té PostCondition o exitCondition.
		 * 
		 * PAS 6: Amb el ADLNAV i la resposta de les Post/ExitConditions
		 * 		intentarem buscar el següent item.
		 * PAS 6.1: Per cada item comprovar el preCondition.
		 * 
	...
	private boolean test1() {
		...
		Double resultMeasure =
			org.objectiveMeasure(cUseObject);
	...
	private boolean test2() {
		...
		SequencingStatus resultAttmpt =
			org.attempted(cUseObject);
		if (resultAttmpt == null) {
			return false;
		} else {
			System.out.println("\ttest2->attempted:" 
					+ resultAttmpt.toString());
		}
		
		SequencingStatus resultCmplt =
			org.completed(cUseObject, true);
		if (resultCmplt == null) {
			return false;
		} else {
			System.out.println("\ttest2->completed:" 
					+ resultCmplt.toString());
		}
		
		SequencingStatus resultStisf =
			org.satisfied(cUseObject, true);
		if (resultStisf == null) {
			return false;
		} else {
			System.out.println("\ttest2->satisfied:" 
					+ resultStisf.toString());
		}
		
		SequencingStatus resultObjMKn =
			org.objectiveMeasureKnown(cUseObject);
		if (resultObjMKn == null) {
			return false;
		} else {
			System.out.println("\ttest2->objectiveMeasureKnown:" 
					+ resultObjMKn.toString());
		}
		
		SequencingStatus resultObjSKn =
			org.objectiveStatusKnown(cUseObject);
		if (resultObjSKn == null) {
			return false;
		} else {
			System.out.println("\ttest2->objectiveStatusKnown:" 
					+ resultObjSKn.toString());
		}
	...
	private boolean test3() {
		...
		for (Iterator it = org.getChildrenIterator();
			it.hasNext();) {
		...
		SequencingStatus resultAttmpt =
			module1.attempted(cUseObject);
		if (resultAttmpt == null) {
			retBool = false;
			System.out.println("\ttest3->attempted: NULL");
		} else {
			System.out.println("\ttest3->attempted:" 
					+ resultAttmpt.toString());
		}
		
		SequencingStatus resultObjMKn =
			module1.objectiveMeasureKnown(cUseObject);
		if (resultObjMKn == null) {
			retBool = false;
			System.out.println("\ttest3->objectiveMeasureKnown: NULL");
		} else {
			System.out.println("\ttest3->objectiveMeasureKnown:" 
					+ resultObjMKn.toString());
		}
		
		SequencingStatus resultObjSKn =
			module1.objectiveStatusKnown(cUseObject);
		if (resultObjSKn == null) {
			retBool = false;
			System.out.println("\ttest3->objectiveStatusKnown: NULL");
		} else {
			System.out.println("\ttest3->objectiveStatusKnown:" 
					+ resultObjSKn.toString());
		}
		
		SequencingStatus resultStisf =
			module1.satisfied(cUseObject, true);
		if (resultStisf == null) {
			retBool = false;
			System.out.println("\ttest3->satisfied: NULL");
		} else {
			System.out.println("\ttest3->satisfied:" 
					+ resultStisf.toString());
		}
		
		SequencingStatus resultCmplt =
			module1.completed(cUseObject, true);
		if (resultCmplt == null) {
			retBool = false;
	...
	private boolean test4() {
		...
		Root_Item exam = null;
		for (Iterator it = exammenu.getChildrenIterator();
			it.hasNext();) {
			Abstract_Item abstSon = (Abstract_Item) it.next();
		...
		for (Iterator it = exam.getChildrenIterator();
			it.hasNext();) {
			Abstract_Item abstSon = (Abstract_Item) it.next();
		...
		SequencingStatus resultAttmpt =
			exam3.attempted(cUseObject);
		if (resultAttmpt == null) {
			retBool = false;
			System.out.println("\ttest4->attempted: NULL");
		} else {
			System.out.println("\ttest4->attempted:" 
					+ resultAttmpt.toString());
		}
		
		SequencingStatus resultObjMKn =
			exam3.objectiveMeasureKnown(cUseObject);
		if (resultObjMKn == null) {
			retBool = false;
			System.out.println("\ttest4->objectiveMeasureKnown: NULL");
		} else {
			System.out.println("\ttest4->objectiveMeasureKnown:" 
					+ resultObjMKn.toString());
		}
		
		SequencingStatus resultObjSKn =
			exam3.objectiveStatusKnown(cUseObject);
		if (resultObjSKn == null) {
			retBool = false;
			System.out.println("\ttest4->objectiveStatusKnown: NULL");
		} else {
			System.out.println("\ttest4->objectiveStatusKnown:" 
					+ resultObjSKn.toString());
		}
		
		SequencingStatus resultStisf =
			exam3.satisfied(cUseObject, true);
		if (resultStisf == null) {
			retBool = false;
			System.out.println("\ttest4->satisfied: NULL");
		} else {
			System.out.println("\ttest4->satisfied:" 
					+ resultStisf.toString());
		}
		
		SequencingStatus resultCmplt =
			exam3.completed(cUseObject, true);
		if (resultCmplt == null) {
			retBool = false;
		...
		Double resultMeasure =
			exam3.objectiveMeasure(cUseObject);
		if (resultCmplt == null) {
			retBool = false;
			System.out.println("\ttest4->objectiveMeasure: NULL");
		} else {
			System.out.println("\ttest4->objectiveMeasure:" 
					+ resultMeasure);
		}		
	...
	private boolean test5() {
		...
		System.out.println("\t[TOC1] -> limitCondition ? -> " 
				+ org.getSequencing().limitCondition(cUseObject));
		...
		System.out.println("\t[INTRO] -> limitCondition ? -> " 
				+ intro.getSequencing().limitCondition(cUseObject));
	...
	private boolean test6() {
		...
		ruleActionType tipusReturn =
			intro.getSequencing().preConditionRule(cUseObject);
		if (tipusReturn == null) {
			System.out.println("\t[INTRO] -> preConditionRule -> NULL");
		} else {
			System.out.println("\t[INTRO] -> preConditionRule -> " 
					+ tipusReturn.toString());
		}
	...
	private boolean test7() {
		 ...
		 */
		Stack < String > route = 
			cManager.searchRouteMap(cUseObject, "QUESTION9A");		
		if (route == null) {
			retBool = false;
			System.out.println("\t[CouseManager] -> searchRouteMap -> NULL");
		} else {
			System.out.println("\t[CouseManager] -> searchRouteMap -> " 
					+ route.toString());					
		}		
		/**
		 * PAS 2: "Accedir" a l'Item amb el camí descobert.
		...
		if (lItem == null) {
			...
			System.out.println("\t[CouseManager] -> searchMyItem -> " 
					+ "(SCO? " + lItem.getIsSCO() + ") " 
		...
		/**
		 * TODO: Fataria actualitzar el StudentsClass.last_item_title
		 */
		/**
		 * PAS 4: Actualitzar els ClusterMaps del pare->avi->...->
		 * 		organització de tal manera que només s'actualitzi
		 ...
		 */
		ruleActionType rAT = 
			lItem.getSequencing().postConditionRule(cUseObject);
		if (rAT != null) {
			System.out.println("\t[CouseManager] -> postConditionRule -> "
					+ rAT.toString());
			/**
			 * PAS 5.1: Si té PostCondició analitzarem de que és tracta i
			 * buscarem el següent item.
			...
			if (rAT.equals(ruleActionType._continue)) {
				nouItem =
					lItem.nextItem(cUseObject, lItem.getIdentifier());
			} else if (rAT.equals(ruleActionType.previous)) {
				nouItem =
					lItem.previousItem(cUseObject, lItem.getIdentifier());
			} else if (rAT.equals(ruleActionType.retry)) {
				if (lItem.father != null) {
					nouItem =
						((Root_Item) lItem.father).firstLeafItem(cUseObject);
				} else {
					System.out.println("[ERROR] Ens hem trobat que '" 
							+ lItem.getIdentifier() + "' no te pare!!");
				}
			} else if (rAT.equals(ruleActionType.retryAll)) {
				nouItem =
					((Root_Item) cManager.getOrganizationCluster()).
						firstLeafItem(cUseObject);
			} else if (rAT.equals(ruleActionType.exitParent)) {
				if (lItem.father != null) {
					nouItem =
						lItem.father.nextItem(cUseObject,
				...
				/**
				 * TODO: Tractar els Exits!Guardar-ho tot a la BD?
				 */
			...
			if (cUseObject.equals("continue")) {
				...
				nouItem =
					lItem.previousItem(cUseObject, lItem.getIdentifier());
			} else if (cUseObject.adlnav.request.equals("exit") 
					|| cUseObject.adlnav.request.equals("exitAll")
					|| cUseObject.adlnav.request.equals("abandon")
					|| cUseObject.adlnav.request.equals("abandonAll")
					|| cUseObject.adlnav.request.equals("suspendAll")) {
				/**
				 * TODO: Tractar els Exits!Guardar-ho tot a la BD?
				 */												
			} else if (cUseObject.adlnav.request.equals("_none_")) {
				cUseObject.adlnav.lmsException = "Hem rebut un _none_ " 
					+ "com a request!";
				...
				StringTokenizer tokenizer =
					new StringTokenizer(cUseObject.adlnav.request, "{");
	            ...
	            if (idNewItem == null) {
	            	cUseObject.adlnav.lmsException = "Identificador " 
	            		+ "d'Item incorrecte!";
	            } else {
	            	/**
	        		...
	        		Stack < String > routeNI =
	        			cManager.searchRouteMap(cUseObject, idNewItem);
	 ...
	 */
	private void entryUserObjectiveTestData(
			final UserObjective cUserObject,
			final CourseManager nouCourseManager) {
		UserObjective rUObject = cUserObject;
		Calendar calendar = new GregorianCalendar();
		
		/*
		Duration durada = null;
		try {
			durada = DatatypeFactory.newInstance().newDuration("P1Y3M2DT3H");
		...
		*/
		...
		rUObject.dataModel.get("INTRO").firstAttemptedTime =
			calendar.getTime();		
		rUObject.dataModel.get("INTRO").activityAttemptCount = 8;
		rUObject.dataModel.get("INTRO").totalTime = "PT8M";
		calendar.add(Calendar.DATE, +1);
		rUObject.dataModel.get("INTRO").lastAttemptedTime =
			calendar.getTime();
		rUObject.treeAnnotations.get("INTRO").isAccessed = true;
		
		//Actualitzem les dades per INTRO
		Stack < String > route = 
			nouCourseManager.searchRouteMap(cUseObject, "INTRO");
		...
		//		Actualitzem les dades per LESSON1
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "LESSON1");
		lItem = null;
		...
		//		Actualitzem les dades per LESSON2
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "LESSON2");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject, route);
		lItem.overallRollupProcess(rUObject);
		...
		//		Actualitzem les dades per LESSON3
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "LESSON3");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		rUObject.dataModel.get("LESSON4").firstAttemptedTime =
			calendar.getTime();		
		rUObject.dataModel.get("LESSON4").activityAttemptCount = 1;
		rUObject.dataModel.get("LESSON4").totalTime = "PT1M";
		calendar.add(Calendar.DATE, +1);
		rUObject.dataModel.get("LESSON4").lastAttemptedTime =
			calendar.getTime();
		rUObject.treeAnnotations.get("LESSON4").isAccessed = true;
		//		Actualitzem les dades per LESSON4
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "LESSON4");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		rUObject.dataModel.get("LESSON7B").firstAttemptedTime =
			calendar.getTime();			
		rUObject.dataModel.get("LESSON7B").activityAttemptCount = 8;
		rUObject.dataModel.get("LESSON7B").totalTime = "PT8M";
		calendar.add(Calendar.DATE, +1);
		rUObject.dataModel.get("LESSON7B").lastAttemptedTime =
			calendar.getTime();
		rUObject.treeAnnotations.get("LESSON7B").isAccessed = true;
		//		Actualitzem les dades per LESSON7B
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "LESSON7B");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("1LEAFOBJJ").completionStatus = "completed";
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("1LEAFOBJJ").successStatus = "passed";
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("1LEAFOBJJ").scoreScaled = "0.7";
		
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("2LEAFOBJJ").completionStatus = "completed";
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("2LEAFOBJJ").successStatus = "passed";
		rUObject.dataModel.get("QUESTION1").
			cmiObjectives.get("2LEAFOBJJ").scoreScaled = "1";			
		/*
		...
		*/
		...
		//		Actualitzem les dades per QUESTION1
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION1");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		//		Actualitzem les dades per QUESTION2
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION2");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		//		Actualitzem les dades per QUESTION7
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION7");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		//		Actualitzem les dades per QUESTION8
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION8");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		//		Actualitzem les dades per QUESTION9
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION9");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		rUObject.dataModel.get("QUESTION9A").firstAttemptedTime =
			calendar.getTime();		
		...
		calendar.add(Calendar.DATE, +1);
		rUObject.dataModel.get("QUESTION9A").lastAttemptedTime =
			calendar.getTime();
		rUObject.treeAnnotations.get("QUESTION9A").isAccessed = true;
		//		Actualitzem les dades per QUESTION9A
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION9A");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		rUObject.dataModel.get("QUESTION9B").firstAttemptedTime =
			calendar.getTime();		
		...
		calendar.add(Calendar.DATE, +1);
		rUObject.dataModel.get("QUESTION9B").lastAttemptedTime =
			calendar.getTime();
		rUObject.treeAnnotations.get("QUESTION9B").isAccessed = true;
		//		Actualitzem les dades per QUESTION9B
		route = null;
		route = 
			nouCourseManager.searchRouteMap(rUObject, "QUESTION9B");
		lItem = null;
		lItem = nouCourseManager.searchMyItem(cUserObject,route);
		lItem.overallRollupProcess(rUObject);
		...
		/**
		 * Estem passant per referéncia!!! no cal retornar :D 
		 */
		//return rUObject;
	}
