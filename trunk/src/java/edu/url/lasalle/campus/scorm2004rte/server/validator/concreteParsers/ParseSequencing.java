...
import edu.url.lasalle.campus.scorm2004rte.server.
	ActivityTree.Elements.Objective;
import edu.url.lasalle.campus.scorm2004rte.server.
	ActivityTree.Elements.ObjectiveCluster;
import edu.url.lasalle.campus.scorm2004rte.server.
	ActivityTree.Elements.ObjectiveHandler;
import edu.url.lasalle.campus.scorm2004rte.server.
...
import edu.url.lasalle.campus.scorm2004rte.server.
	DataBase.DynamicData.ClusterDataModel;
import edu.url.lasalle.campus.scorm2004rte.server.
	DataBase.DynamicData.ClusterMap;
 ...
 * $Id: ParseSequencing.java,v 1.15 2008/01/08 11:50:21 ecespedes Exp $
 * 
 * Aquesta classe parseja el seqüenciament i crida les subclasses corresponents
 * a cada element del seqüenciament (controlMode, Objectives, 
 * ConditionRule, etc).
 * 
 * @author ecespedes
 * @version Versió $Revision: 1.15 $ $Date: 2008/01/08 11:50:21 $
...
public class ParseSequencing {
	...
	/**
	 * Aquesta variable ens indicarà si REALMENT hem trobat 
	 * alguna cosa (etiqueta, regla, rollup, etc) de seqüenciament,
	 * o si pel contrari estem retornant el seqüenciament per 
	 * defecte.
	 */
	private boolean hasReallyANYSequencing = false;
		
	/**
	 * Guardarem totes les regles del seqüenciament dintre de la 
	 * classe Sequencing. 
	 */	
	public Sequencing sequencing = new Sequencing();
	/**
	 * Guardarem tots els identificadors dels objectius 
	 * (tan primaris com secundaris) dintre CMIDataModel. 
	 */
	//public CMIDataModel cmiDataModel;
	/**
	 * Ens servirà per indicar si té Objectives o no.
	 */
	...
	/**
	 * És el parsejador del seqüenciament.
	 * 
	 ...
	 */
	public ParseSequencing(final Node sequencingNode,
			final CMIDataModel cmiDataModel,
			final ClusterMap clusterMap,
			final String currentItemID,
			final boolean isLeafItem,
			final ClusterMap globalMap) {
		
		itemID = currentItemID;		
		//cmiDataModel = nouCMIDataModel;
		
		//parsegem els atributs, si és que en té. 
		String iD = 
			DOMTreeUtility.getAttributeValue(sequencingNode, Constants.ID);
		...
		String iDRef = 
			DOMTreeUtility.getAttributeValue(sequencingNode, Constants.IDREF);
				...
				if (ssNode == null) {
					if (Constants.DEBUG_ERRORS || Constants.DEBUG_SEQUENCING) {
						System.out.println("[ERROR] " 
								+ "a l'agafar un fill dintre del Sequencing!!");
					}
				} else {
					hasReallyANYSequencing = true;
					analizeNode(
							cmiDataModel, clusterMap, 
							isLeafItem, ssNode, globalMap);
				}
			}
		}			
	}
	...
	/**
	 * Analitza els "fills" del seqüenciament i 
	 * crida als parsejadors concrets.
	 * @param childNode : (Node Type)
	 */
	private void analizeNode(
			final CMIDataModel cmiDataModel,
			final ClusterMap clusterMap,
			final boolean isLeafItem,
		...
		if (childName.equals(CONTROLMODE)) {
			...
			if (isLeafItem) {
				sequencing.objectiveHandler = 
					parseObjectives.getObjectiveHandler();
			} else {
				ObjectiveCluster tmpObjCluster = new ObjectiveCluster();
				Objective tmpObjective = 
					(Objective) parseObjectives.getObjectiveHandler()
				.getPrimaryObjective();
				
				tmpObjCluster.itemID = tmpObjective.itemID;
				tmpObjCluster.ObjectiveID = tmpObjective.ObjectiveID;
				tmpObjCluster.minNormalizedMeasure = 
					tmpObjective.minNormalizedMeasure;
				tmpObjCluster.satisfiedByMeasure = 
					tmpObjective.satisfiedByMeasure;
				...
				if (tmpObjCluster.mapInfo != null) {
					if (tmpObjCluster.mapInfo.targetObjectiveID != null) {
						globalMap.objective.put(tmpObjCluster.mapInfo.targetObjectiveID,
								new ClusterDataModel());
					}
				}
				//parseObjectives.getObjectiveHandler().
					//setPrimaryObjective(tmpObjCluster);
				sequencing.objectiveHandler.
					setPrimaryObjective(tmpObjCluster);
				for (Iterator it = 
					parseObjectives.getObjectiveHandler().
					...
					tmpSC.satisfiedByMeasure = 
						tmpSO.satisfiedByMeasure;
					tmpSC.hasMapInfo = tmpSO.hasMapInfo;
					tmpSC.mapInfo = tmpSO.mapInfo;
					
					sequencing.objectiveHandler.
						addObjective(tmpSC.ObjectiveID, tmpSC);
				}
			}
			
			hasObjectives = true;
			/**
			 * Ara omplirem les dades del CMIDataModel i del CMIObjectives a 
			 * partir del objectiveHandler.
			 */
			CMIObjectives nouCMIObjectives = null;
			//cmiDataModel.cmiObjectives.add(nouCMIObjectives);
			if (isLeafItem) {
				Objective tmpObjective = (Objective) sequencing.
					objectiveHandler.getPrimaryObjective();
				
				nouCMIObjectives = new CMIObjectives();
				nouCMIObjectives.id = tmpObjective.ObjectiveID;
				
				cmiDataModel.cmiObjectives.put(
						nouCMIObjectives.id, nouCMIObjectives);
			} else {
				ObjectiveCluster tmpObjective = 
					(ObjectiveCluster) sequencing.objectiveHandler.
						getPrimaryObjective();
				ClusterDataModel tmpClusterDM = new ClusterDataModel();
				clusterMap.objective.put(
						tmpObjective.ObjectiveID, tmpClusterDM);
			}
						
			for (Iterator iteratorObjectives = 
				sequencing.objectiveHandler.getObjectivesIterator();
					iteratorObjectives.hasNext();) {
				if (isLeafItem) {
					Objective tmpObjective = 
						(Objective) iteratorObjectives.next();
					nouCMIObjectives = new CMIObjectives();
					nouCMIObjectives.id = tmpObjective.ObjectiveID;
					//cmiDataModel.cmiObjectives.add(nouCMIObjectives);
					cmiDataModel.cmiObjectives.put(
					...
					if (tmpObjective.mapInfo != null) {
						if (tmpObjective.mapInfo.targetObjectiveID != null) {
							globalMap.objective.put(tmpObjective.mapInfo.targetObjectiveID,
									new ClusterDataModel());
						}
					}
				} else {
					ObjectiveCluster tmpObjective = 
						(ObjectiveCluster) iteratorObjectives.next();
					ClusterDataModel tmpClusterDM = new ClusterDataModel();
					clusterMap.objective.put(
							tmpObjective.ObjectiveID, tmpClusterDM);
					if (tmpObjective.mapInfo != null) {
			...
			ParseConditionRule nouParseConditionRule = 
				new ParseConditionRule(childNode, sequencing);
			...
			String attemptLimit = 
				DOMTreeUtility.getAttributeValue(childNode, "attemptLimit");
			...
			ParseRandomizeControls nouParseRandomizeControls =
				new ParseRandomizeControls(childNode, sequencing);
			...
			if (!nouParseRandomizeControls.getIsAllCorrect()) {
				...
				if (Constants.DEBUG_INFO || Constants.DEBUG_SEQUENCING) {
					System.out.println("\t[RandomizationControls-START]\n\t\t" 
							+ "reorderChildren: " 
							+ sequencing.reorderChildren + "\n\t\t"
							+ "randomizationTiming: "
							+ sequencing.randomizationTiming.toString() 
							+ "\n\t\t"
			...
			ParseRollupConsiderations nouParseRollupConsiderations =
				new ParseRollupConsiderations(childNode);
			
			if (nouParseRollupConsiderations.getIsAllCorrect()) {
				sequencing.rollupConsiderations = 
					nouParseRollupConsiderations.rollupConsiderations;
				...
				if (Constants.DEBUG_INFO || Constants.DEBUG_SEQUENCING) {
					System.out.println("\t[RollupConsiderations-START]\n\t\t" 
							...
							+ sequencing.rollupConsiderations.
								requiredForCompleted.toString() 
							+ "\n\t\t"
							+ "requiredForIncomplete: "
							+ sequencing.rollupConsiderations.
								requiredForIncomplete.toString()
							+ "\n\t\t"
							+ "requiredForSatisfied: "
							+ sequencing.rollupConsiderations.
								requiredForSatisfied.toString() 
							+ "\n\t\t" + "requiredForNotSatisfied: "
							+ sequencing.rollupConsiderations.
								requiredForNotSatisfied.toString()
							+ "\n\t" + "[RollupConsiderations-END]");
				}				
			} else {
				if (Constants.DEBUG_ERRORS || Constants.DEBUG_SEQUENCING) {
					System.out.println("[ERROR] L'Element " 
							+ "RollupConsiderations ha retornat un " 
							+ "error inesperat.");
				}
			}
			...
			String tracked = 
				DOMTreeUtility.getAttributeValue(childNode, "tracked");
			...
			if (tracked.length() == 0) {			
				...
				hasReallyANYSequencing = true;
				sequencing.tracked = 
					tracked.equals(Constants.FALSE) ? false : true;
			}			
			if (Constants.DEBUG_INFO || Constants.DEBUG_SEQUENCING) {
	...
	/**
	 * Ens indicarà si té o no té IDRef. 
	 * (default false).
	 * 
	 * @return boolean : 
	 */
	...
	/**
	 * Ens indicarà si té o no té identificador. 
	 * (default false).
	 * 
	 * @return boolean 
	 */
	...
	/**
	 * Ens indicarà si té o no té Objectius. 
	 * (default false).
	 * 
	 * @return boolean 
	 */
	...
	/**
	 * Aquesta funció ens indicarà si REALMENT hem trobat 
	 * alguna cosa (etiqueta, regla, rollup, etc) de seqüenciament,
	 * o si pel contrari estem retornant el seqüenciament per 
	 * defecte.
	 * 
